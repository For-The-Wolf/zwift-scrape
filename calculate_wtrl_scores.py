...
def appendScores(finishes, primes, exclude):
    ...
    totals = [
        r["Finish"] + r["First over line"] + r["Fastest time"]
...
def main():
    parser = ArgumentParser(
        description="Calculate WTRL scores from zwift-power URL or CSV."
    )
    parser.add_argument(
        "datapath",
        help="This can be either a URL to zwift results or a filepath to the output from zwift_scrape.",
    )
    parser.add_argument(
        "--saveName",
        "-s",
        help="Name to save output data to. If not specified then scraped race title will be used",
    )
    parser.add_argument(
        "--ptsFirst",
        "-p",
        action="store_true",
        help="Sort output by score, otherwise sorts by finish position.",
    )
    parser.add_argument(
        "--onlyScores",
        "-o",
        action="store_true",
        help="Only save CSV of scores (do not save scraped times and primes).",
    )
    parser.add_argument(
        "--excludeSplit",
        "-e",
        action="append",
        default=[],
        help="Exclude a split from the scoring, can be called any number of times.",
    )
